...
impl Channel {
    ...
    fn wait_recv(&mut self) -> Option<(u32, u32)> {
        ...
        loop {
            ...
            if recved == sended {
                return Some((sended, recved));
            }

            if begin.elapsed() > SYNC_TIME_OUT {
                return None;
            }
        }
    }
    ...
    fn recv(&mut self) -> Frame {
        ...
        self.data_ty.store(FRAME_NONE, Ordering::Relaxed);
        result
    }
}
...
impl ShmAdaptor {
    ...
    pub fn wait(&self, timeout: Option<Timeout>) -> bool {
        self.shmem()
            .wait(Self::EVT_MASTER, timeout.unwrap_or(Timeout::Infinite))
            .is_ok()
    }
