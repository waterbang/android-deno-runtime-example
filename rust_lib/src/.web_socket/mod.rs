...
//! `web socket` is a cross-language RPC framework.
//! # 示例
//! ```
//! use std::sync::Arc;
//! use easy_rpc::*;
...
use rmps::{decode::Error as DecodeError, Serializer};
...
impl RequestResult {
    pub fn into<T: DeserializeOwned>(self) -> Result<T, RequestResult> {
        match self {
            RequestResult::Data(d) => {
                rmps::from_read_ref(d.as_slice()).map_err(|_| RequestResult::Decode(d))
            }
    ...
    #[inline]
    pub fn intos<T: DeserializeOwned>(self) -> Result<T, String> {
...
impl<'a> Arg<'a> {
    #[inline]
    pub fn into<T>(self) -> Result<T, DecodeError>
    where
        T: DeserializeOwned,
    {
...
// impl<T> std::ops::FnOnce<(T,)> for Ret<'_, '_>
// where
//     T: Serialize,
// {
//     type Output = ();
...
//             RequestResult::Error(err) => self.error(&err),
//             _ => {}
//         }
//     }
// }
...
impl<'a, 'b> Ret<'a, 'b> {
    ...
    pub unsafe fn into_async(self) -> Option<AsyncRet> {
        self.req_id.map(|req_id| AsyncRet {
            ss: self.ss.arc_clone(),
            req_id,
        })
    }
...
// impl<T> std::ops::FnOnce<(T,)> for AsyncRet
// where
//     T: Serialize,
// {
//     type Output = ();
...
impl ToMethod<'_> for u32 {
    #[inline(always)]
    fn to_method(self) -> Method<'static> {
        Method::Int(self)
    }
}
...
impl Session {
    ...
    pub unsafe fn request_transfer<'a>(
        &self,
        method: impl ToMethod<'a>,
        msgpack: &[u8],
    ) -> RequestResult {
        let (mut pack, req_id) = self.prepare_request(method.to_method());
